...
impl GraphicsHandle {
    ...
    fn draw_subimage_scaled_normalized(
        ...
        match self {
            Self::GL(g) => g.draw_subimage_scaled_normalized(texture, x, y, w, h, sx, sy, sw, sh),
        }
    }

    pub fn draw_image(&mut self, texture: Texture, x: f32, y: f32) {
        self.draw_subimage_scaled_normalized(
            texture,
            x,
            y,
    ...
    pub fn draw_subimage(
        ...
        self.draw_subimage_scaled(texture, x, y, sw, sh, sx, sy, sw, sh);
    }

    pub fn draw_image_scaled(&mut self, texture: Texture, x: f32, y: f32, w: f32, h: f32) {
        self.draw_subimage_scaled_normalized(texture, x, y, w, h, 0.0, 0.0, 1.0, 1.0);
    }

    pub fn draw_subimage_scaled(
        &mut self,
        texture: Texture,
        x: f32,
        y: f32,
...
impl Color {
    const BYTE_FLOAT_LOOKUP_TABLE: [f32; 256] = [
        0.0,
        0.003921569,
        0.007843138,
        0.011764707,
